<email-table>
    ...
    <div if = { serverSelectorRef.serverSelected === true && emails.length > 0 } class="table-responsive">
	    <table class="table table-striped table-bordered table-hover table-condensed">  
	        ...
	        <tbody> 
	            <tr each = { emails }> 
	                <td class="col-md-2">{ receivedOn }</td>
	                <td class="col-md-1">{ sender }</td>
	                <td class="col-md-3"><span each ={ recipients }>{ address }; </span></td>
    ...
    <script>
        ...
        refresh() {
           ...
           $.get('./api/servers/' + serverName + '/emails', function(data) {
                self.emails = data;
                self.update();
            });
        }
        ...
        watch() {
        	...
        	self.watchEventSource = new EventSource('./api/broadcast/email/' + serverName);
        ...
        deleteAllEmails() {
        	 var serverName = serverSelectorRef.getSelectedServer();
             var deleteUrl = './api/servers/' + serverName + '/emails';
             $.ajax({
                url: deleteUrl,
                type: 'DELETE',
        ...
        ajaxDeleteEmail(emailToDelete) {
        	 var serverName = serverSelectorRef.getSelectedServer();
        	 var deleteUrl = './api/servers/' + serverName + '/emails/' + emailToDelete;
        	 $.ajax({
       		    url: deleteUrl,
       		    type: 'DELETE'
       		});
